...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@ashnewar/helper-library';
...
const seed = async (req: Request, res: Response): Promise<void> => {
  ...
  const buyers: IBuyerDocument[] | null = await getRandomBuyers(parseInt(count, 10));
  for(let i = 0; i < buyers.length; i++) {
    ...
    if (checkIfSellerExist) {
      throw new BadRequestError('Seller already exist.', 'SellerSeed seed() method error');
    }
